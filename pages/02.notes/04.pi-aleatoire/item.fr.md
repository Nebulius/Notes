...
La valeur de la probabilité est juste, certes, c'est bien $6/\pi^2$. Mais le fait est que cette probabilité vaut $6/\pi^2$ *uniquement si on tire un nombre dans l'ensemble des entiers positifs non nuls*. Plus formellement, si on note $P_N$ la probabilité que deux nombres entiers choisis au hasard _inférieurs à $N$_ soient premiers entre eux, alors

$$\lim\limits_{N \to \infty} P_N = \frac{6}{\pi^2}$$

Le fait est que **la convergence est extrêmement lente**. On l'a d'ailleurs vu dans la première partie : avec 1000 tirages, on obtient péniblement une décimale. Deux avec un million. Quatre seulement avec *dix milliards*. Pourquoi ? Car on est très, très, très loin de tirer des nombres aléatoires sur tous les nombres possibles.

Si on le fait au dé, on peut tirer avec des gros dés sur des nombres inférieurs à 200 — et on obtient entre zéro et une décimale. Dans mon cas, comme j'ai utilisé un ordinateur pour faire les tirages, j'ai pris comme maximum la plus grande valeur que j'ai pu, à savoir la valeur maximale d'un entier non-signé de 128 bits, pour ceux à qui ça parle. Ce nombre vaut environ $3 \times 10^{38}$, ou exactement :

$$340\ 282\ 366\ 920\ 938\ 463\ 463\ 374\ 607\ 431\ 768\ 211\ 455$$

C'est grand, très grand, tellement grand qu'il est difficile de se l'imaginer ! Et c'est pourtant ridiculement petit face à l'infinité des nombres entiers. Comme on l'a constaté, c'est très loin d'être suffisant pour calculer π avec plus de quelques décimales.
